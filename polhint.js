...
function init() {
  ...
  program
    ...
    .option('-c, --config [file_name]', 'file to use as your .polhint.json')
    .option('-q, --quiet', 'report errors only - default: false')
    .option('--ignore-path [file_name]', 'file to use as your .polhintignore')
    .description('Linter for Polynomial programming language')
  ...
  program
    .command('init-config')
    .description('create in current directory configuration file for polhint')
...
function execMainAction() {
  ...
  if (printReports(reports, formatterFn)) {
    if (program.maxWarnings && !reports[0].errorCount && warningsNumberExceeded) {
      console.log(
        'Polhint found more warnings than the maximum specified (maximum: %s)',
        program.maxWarnings
      )
      process.exit(1)
    }
...
const readConfig = _.memoize(() => {
  ...
  try {
    const configFile = program.config || '.polhint.json'
    ...
    if (e instanceof SyntaxError) {
      console.log('ERROR: Configuration file [.polhint.json] is not a valid JSON!\n')
    }
