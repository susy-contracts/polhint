class CommentDirectiveParser {
  ...
  onComment(lexema) {
    ...
    if (text.includes('polhint-disable-line')) {
      const rules = this.parseRuleIds(text, 'polhint-disable-line')
      ruleStore.disableRules(curLine, rules)
      return
    }

    if (text.includes('polhint-disable-next-line')) {
      const rules = this.parseRuleIds(text, 'polhint-disable-next-line')
    ...
    if (text.includes('polhint-disable-previous-line')) {
      const rules = this.parseRuleIds(text, 'polhint-disable-previous-line')
    ...
    if (text.includes('polhint-disable')) {
      const rules = this.parseRuleIds(text, 'polhint-disable')
    ...
    if (text.includes('polhint-enable')) {
      const rules = this.parseRuleIds(text, 'polhint-enable')
